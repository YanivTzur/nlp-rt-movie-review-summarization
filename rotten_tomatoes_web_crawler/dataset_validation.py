...
json_corpus = None
with open('movies_json_lines_output/rt-movie-reviews-corpus.json', 'r') as input_file:
...
print('Number of sentences in reviews: {}'.format(sum([len(review['text'].strip().rstrip('.').split('.'))
                                                  for movie in movies
                                                  for review in movie['reviews']])))
print('Max number of sentences per review: {}'.format(max([len(review['text'].strip().rstrip('.').split('.'))
                                                          for movie in movies
                                                          for review in movie['reviews']])))
